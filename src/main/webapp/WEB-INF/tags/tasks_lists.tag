...
<div id="lists">
	<div id="listsTop" class="subTabmenu tabmenu inactiveSubpage h2">
		<div class="tab left">
			<a href="tasks/reset?tab=general"><fmt:message
					key="tasks.menu.general" /><span></span></a>
		</div>
		<div class="tab left">
			<a href="tasks/reset?tab=textures"><fmt:message
					key="tasks.menu.textures" /><span></span></a>
		</div>
		<div class="tab right">
			<a href="tasks/reset?tab=models"><fmt:message
					key="tasks.menu.models" /><span></span></a>
		</div>
		<div class="clear"></div>
	</div>
	<div id="listsMain" class="subTabbody tabbody activeSubpage">
		<div class="search listElement">
			<form:form id="searchForm" method="POST"
				action="/GMM/tasks/submitSearch?tab=${tab}" commandName="search">
				<div class="right switchSearchButton button pageButton"
				...
				<div class="complexSearch">
					<div class="clear"></div>
					<div class="searchDescription left">
						<fmt:message key="tasks.label" />
						:
					</div>
		...
		<div class="elementCount center">
			...
			<fmt:message key="elements" />
			----------------------------------------
		</div>
		<c:forEach items="${taskList}" var="task">
			<div id="${task.getIdLink()}" class="listElement">
				<div class="listElementTop clickable"
					onclick="switchListElement(this)">
					<%-- Priority --%>
...
<%-- Assigned --%>
				...
				<div class="listElementBody">
					...
					<div class="elementDetails elementContent">
						${fn:replace(fn:escapeXml(task.getDetails()), newLine, "<br>")}</div>
					...
					<div class="elementComments">
						...
						<form:form class="commentInput input" method="POST"
							action="/GMM/tasks/submitComment/${task.getIdLink()}?tab=${tab}"
							commandName="comment">
					...
					<c:if test="${tab.equals('textures')}">
						<div class="elementPreview elementContent">
							...
							<div class="subElementPreview left">
								<a
									href="/GMM/tasks/preview?small=false&amp;ver=original&amp;id=${task.getIdLink()}">
									<img
									src="/GMM/tasks/preview?small=true&amp;ver=original&amp;id=${task.getIdLink()}"
									alt="Original Texture Not Avaliable">
								</a>
							</div>
							<div class="subElementPreview right">
								<a
									href="/GMM/tasks/preview?small=false&amp;ver=newest&id=${task.getIdLink()}">
									<img
									src="/GMM/tasks/preview?small=true&amp;ver=newest&id=${task.getIdLink()}"
									alt="New Texture Not Avaliable">
								</a>
							</div>
							<div class="clear"></div>
						</div>
						...
						<div class="elementFiles elementContent">
							<div class="subElementAssets left">
								<div class="subFilesDescriptor">Assets</div>
								<div id="assetFilesContainer" class="subFilesContainer"></div>
							</div>
							<div class="subElementWip right">
								<div class="subFilesDescriptor">Other</div>
								<div id="wipFilesContainer" class="subFilesContainer"></div>
							</div>
							<div class="clear"></div>
							<%-- File Operations --%>
							<div class="subElementFileOperations">
								<input id="${task.getIdLink()}-upload" type="file"
									style="display: none;"
									onchange="uploadFile(this,'${task.getIdLink()}')" />
								<div class="button subElementButton left"
									onclick="$('#${task.getIdLink()}-upload').click()">
									Upload</div>
								<%-- 								<a id="${task.getIdLink()}-download" href="#">Link</a> --%>
								<div class="button subElementButton left"
									onclick="downloadFile('${task.getIdLink()}')">Download</div>
								<div class="button subElementButton right"
									onclick="confirmDeleteFile('${task.getIdLink()}')">
									Delete</div>
								<div class="clear"></div>
							</div>
						</div>
